...
void Library::addItem(Item* item) { catalog.addItem(item); }
void Library::addUser(User* u) { users.push_back(u); }
void Library::listItems() { catalog.listItems(); }
...
Item* Library::getItem(int index) { return catalog.get(index); }
void Library::listItemsWithIndex() { catalog.listItemsWithIndex(); }


void Library::loanItem(User* user, Item* item, const std::string& start, const std::string& due) {
	if (userBlocked(user)) { std::cout << "Usuario bloqueado por sanciones acumuladas" << std::endl; return; }
...
Loan* Library::findLoan(User* user, Item* item) {
	for (auto l : loans) {
		if (l->getUser() == user && l->getItem() == item && !l->isReturned()) return l;
	}
	return nullptr;
}
...
void Library::returnItem(User* user, Item* item, const std::string& returnDate) {
	...
	if (!loan) { std::cout << "Prestamo no encontrado" << std::endl; return; }
	loan->setReturnDate(returnDate);
	std::cout << "Devolucion registrada correctamente" << std::endl;


	double fine = calculateFineA1(loan->getDue(), returnDate);
	if (fine > 0) {
...
bool Library::userBlocked(User* user) {
	if (user->getSanction() > 10) return true;
	if (user->isManuallyBlocked()) return true;
	return false;
}
...
void Library::saveItems(const std::string& filename) {
	...
	if (!file) { std::cout << "Error al abrir " << filename << std::endl; return; }
...
const std::vector<Item*>& Library::catalogItems() const { return catalog.getItems(); }
...
void Library::saveLoans(const std::string& filename) {
	...
	for (Loan* loan : loans) {
		int itemIndex = findItemIndex(loan->getItem());
...
int Library::findItemIndex(Item* it) {
	...
	for (int i = 0; i < static_cast<int>(list.size()); ++i) if (list[i] == it) return i;
	return -1;
}
...
void Library::removeItem(int index) {
	...
	if (index < 0 || index >= static_cast<int>(items.size())) { std::cout << "Índice inválido.\n"; return; }
	Item* target = items[index];
	for (Loan* l : loans) {
		if (l->getItem() == target && !l->isReturned()) { std::cout << "No se puede eliminar: el item tiene un préstamo activo.\n"; return; }
	}
	delete items[index];
...
void Library::loadItems(const std::string& filename) {
	...
	if (!file) { std::cout << "No se pudo abrir " << filename << std::endl; return; }
	std::string line;
	...
	while (std::getline(file, line)) {
		...
		if (type == "Book") {
			...
			std::string title = cols.size() > 1 ? cols[1] : "";
			int volume = cols.size() > 3 ? std::stoi(cols[3]) : 0;
...
void Library::loadUsers(const std::string& filename) {
	...
	while (std::getline(file, line)) {
		if (line.empty()) continue;
		auto cols = dividirCSV(line);
		int id = cols.size() > 0 ? std::stoi(cols[0]) : 0;
		std::string name = cols.size() > 1 ? cols[1] : "";
		int role = cols.size() > 2 ? std::stoi(cols[2]) : 0;
...
void Library::sortByTitle(std::vector<Item*>& vec) {
	std::sort(vec.begin(), vec.end(), [](Item* a, Item* b) {
		...
		if (Book* ba = dynamic_cast<Book*>(a)) titleA = ba->getTitle();
		else if (Journal* ja = dynamic_cast<Journal*>(a)) titleA = ja->getTitle();
		else titleA = a->info();
		if (Book* bb = dynamic_cast<Book*>(b)) titleB = bb->getTitle();
		else if (Journal* jb = dynamic_cast<Journal*>(b)) titleB = jb->getTitle();
		else titleB = b->info();
...
void Library::exportActiveLoansCSV(const std::string& filename) {
	std::ofstream file(filename);
	if (!file) { std::cout << "Error al abrir archivo " << filename << std::endl; return; }
...
void Library::blockUser(int userId) {
	User* u = findUser(userId);
	if (!u) { std::cout << "Usuario no encontrado.\n"; return; }
	u->block();
